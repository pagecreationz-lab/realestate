...
export default function Comments({post,onCountChange}:{post:string;onCountChange:(count:number)=>void}){
 ...
 async function load(){const d=await community('comments&post='+encodeURIComponent(post));setComments(d.comments);onCountChange(d.total??d.comments.length);}
 ...
 return <section id={"comments-"+post} className="cs-review-panel"><h3>Comments</h3>{error&&<p role="alert" className="cs-error">{error}</p>}{comments.map(c=><div key={c.id}><strong>{c.name}</strong><small> · {when(c.created_at)}</small><p>{c.body}</p></div>)}<form onSubmit={submit}><label>Add a comment<textarea required minLength={1} maxLength={1000} value={body} onChange={e=>setBody(e.target.value)}/></label><button className="cs-primary" disabled={busy}>Post comment</button></form></section>;
}
